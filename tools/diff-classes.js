...
const refinedPath = path.resolve(__dirname, '..', 'refined-css.md');

if (!fs.existsSync(mainPath)) { console.error('main.css not found:', mainPath); process.exit(2); }
if (!fs.existsSync(refinedPath)) { console.error('refined-css.md not found:', refinedPath); process.exit(2); }
...
console.log('\nFirst 40 only-in-main examples:\n', onlyMain.slice(0,40).join('\n'));
console.log('\nFirst 40 only-in-refined examples:\n', onlyRefined.slice(0,40).join('\n'));
